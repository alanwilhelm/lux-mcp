...
impl CircularReasoningDetector {
    ...
    pub fn extract_concepts(&self, text: &str) -> Vec<Concept> {
        ...
        let noun_indicators = ["the", "a", "an", "this", "that", "these", "those", "how", "what", "why"];
        ...
        for i in 0..tokens.len().saturating_sub(1) {
            ...
            if i + 1 < tokens.len() {
                ...
                let compound_terms = [
                    ...
                    "gradient descent", "back propagation", "chain rule",
                    "learning rate", "activation function", "loss function",
                    "weight update", "error minimization", "feature extraction"
                ];
                ...
                if self.is_meaningful_concept(&tokens[i]) && self.is_meaningful_concept(&tokens[i+1]) {
            ...
            if i + 2 < tokens.len() {
                ...
                if prepositions.contains(&tokens[i+1].as_str()) && self.is_meaningful_concept(&tokens[i]) && self.is_meaningful_concept(&tokens[i+2]) {
                    let compound = format!("{} {} {}", 
                        self.basic_stem(&tokens[i]), 
                        tokens[i+1], 
        ...
        for word in &important_words {
            if self.is_meaningful_concept(word) {
                ...
                let already_in_compound = concepts.iter().any(|c| {
                    match c {
        ...
        for i in 0..tokens.len().saturating_sub(2) {
            if noun_indicators.contains(&tokens[i].as_str()) && self.is_meaningful_concept(&tokens[i+1]) {
        ...
        let action_verbs = ["understand", "understanding", "solve", "calculate", "determine", 
                           "analyze", "process", "need", "require", "requires", "learn", 
                           "learning", "update", "minimize", "improve"];
        for (i, token) in tokens.iter().enumerate() {
            let stemmed_verb = self.basic_stem(token);
            if action_verbs.contains(&token.as_str()) || action_verbs.contains(&stemmed_verb.as_str()) {
    ...
    pub fn detect_pattern(&self, current_thought: &str, thought_history: &[String]) -> CircularPattern {
        ...
        let conceptual_similarity = self.detect_conceptual_loops(&current_concepts, thought_history);
    ...
    fn detect_cyclic_pattern(&self, current_concepts: &[Concept], history: &[String]) -> Option<CircularPattern> {
        if history.len() < 3 {
            return None;
        }
        ...
        if similarity_to_first > 0.7 {
            ...
            let avg_intermediate = intermediate_similarities.iter().sum::<f64>() / intermediate_similarities.len() as f64;
    ...
    fn detect_conceptual_loops(&self, current_concepts: &[Concept], history: &[String]) -> f64 {
        ...
        for past_thought in history.iter().rev().take(5) {
            ...
            let distribution_sim = self.compare_distributions(&current_distribution, &past_distribution);
...
impl ConceptIndex {
    ...
    fn add_document(&mut self, concepts: &[Concept]) {
        ...
        let unique_concepts: HashSet<String> = concepts.iter()
            .map(|c| c.to_string())
            .collect();
...
mod tests {
    ...
    fn test_concept_extraction() {
        ...
        assert!(concepts.iter().any(|c| matches!(c, Concept::Action(s) if s.contains("understand"))));
        assert!(concepts.iter().any(|c| matches!(c, Concept::Single(s) if s == "recursion")));
    }
    ...
    fn test_circular_pattern_detection() {
        ...
        match pattern {
            CircularPattern::Direct { similarity_score, .. } => {
    ...
    fn test_no_circular_pattern() {
        ...
        for (i, h) in history.iter().enumerate() {
            ...
            println!("History[{}] concepts: {:?}, similarity: {}", i, concepts, sim);
        }
