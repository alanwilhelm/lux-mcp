use crate::monitoring::circular_reasoning::{CircularReasoningDetector, Concept};
...
impl DistractorFixationDetector {
    ...
    fn calculate_relevance_score(&self, current: &[Concept], original: &[Concept]) -> f64 {
        ...
        let base_similarity = self
            .concept_extractor
            .calculate_similarity(current, original);
    ...
    fn concepts_related(&self, c1: &Concept, c2: &Concept) -> bool {
        ...
        let ml_relationships: &[(&str, &[&str])] = &[
            ...
            (
                "backpropagation",
                &["neural", "gradient", "chain rule", "derivative", "learning"],
            ),
            (
                "gradient",
                &[
            ...
            (
                "deep learning",
                &["neural", "layer", "convolution", "recurrent", "transformer"],
            ),
            // Programming concepts
            (
                "recursion",
                &["recursive", "base case", "call stack", "self-reference"],
            ),
            (
                "tcp/ip",
                &["network", "protocol", "packet", "internet", "communication"],
            ),
            (
                "network",
                &["communication", "protocol", "tcp", "http", "connection"],
            ),
        ];
        ...
        let words1: Vec<&str> = s1.split_whitespace().filter(|w| w.len() > 3).collect();
        let words2: Vec<&str> = s2.split_whitespace().filter(|w| w.len() > 3).collect();
    ...
    fn detect_pattern(&self) -> DistractorPattern {
        ...
        if self.detail_density_history.len() >= 2 {
            let recent_density: Vec<f64> = self
                .detail_density_history
                .iter()
                .rev()
                .take(3)
                .cloned()
        ...
        if self.relevance_history.len() >= 3 {
            let recent_relevance: Vec<f64> = self
                .relevance_history
                .iter()
                .rev()
                .take(3)
            ...
            let avg_relevance =
                recent_relevance.iter().sum::<f64>() / recent_relevance.len() as f64;
            ...
            {
                println!(
                    "Tangential drift check - recent: {:?}, avg: {}, low_count: {}",
...
fn is_detail_word(word: &str) -> bool {
    ...
    detail_indicators
        .iter()
        .any(|&indicator| word_lower.contains(indicator))
}
...
mod tests {
    ...
    fn test_relevance_drift_detection() {
        ...
        // On-topic thought
        let (distracted, pattern) =
            detector.detect_fixation("TCP/IP uses a four-layer model for network communication");
        assert!(!distracted);
        assert!(matches!(pattern, DistractorPattern::None));

        // Start drifting
        let (_, _) =
            detector.detect_fixation("The internet has evolved significantly since ARPANET");
        let (_, _) =
            detector.detect_fixation("Tim Berners-Lee invented the World Wide Web in 1989");

        // Should detect tangential drift
        let (distracted, pattern) =
            detector.detect_fixation("Social media has transformed how people communicate online");
        println!(
            "Final detection - distracted: {}, pattern: {:?}",
            distracted, pattern
        );
    ...
    fn test_detail_spiral_detection() {
        ...
        let (distracted, _) =
            detector.detect_fixation("Machine learning enables computers to learn from data");
        assert!(!distracted);

        // Gradually increase detail
        let (_, _) = detector.detect_fixation(
            "Specifically, supervised learning uses labeled data with features x1, x2, x3",
        );
        let (_, _) = detector.detect_fixation(
            "The gradient descent algorithm updates weights: w = w - α * ∂L/∂w where α=0.01",
        );
        ...
        println!(
            "Detail spiral test - distracted: {}, pattern: {:?}",
            distracted, pattern
        );
        println!("Relevance history: {:?}", detector.relevance_history);
        println!(
            "Detail density history: {:?}",
            detector.detail_density_history
        );
    ...
    fn test_topic_hopping_detection() {
        ...
        let (_, _) =
            detector.detect_fixation("Photosynthesis converts light energy to chemical energy"); // On topic
        ...
        let (distracted, pattern) =
            detector.detect_fixation("Chloroplasts contain the photosynthetic machinery"); // Back on topic

        println!(
            "Topic hopping test - distracted: {}, pattern: {:?}",
            distracted, pattern
        );
        println!("Relevance history: {:?}", detector.relevance_history);
    ...
    fn test_productive_exploration() {
        ...
        let (distracted, _) =
            detector.detect_fixation("Neural networks learn through backpropagation");
        assert!(!distracted);

        let (distracted, _) =
            detector.detect_fixation("Backpropagation calculates gradients using the chain rule");
        println!(
            "After 'Backpropagation...' - relevance: {:?}",
            detector.relevance_history.back()
        );
        assert!(!distracted);

        let (distracted, _) =
            detector.detect_fixation("This allows networks to update weights to minimize error");
        assert!(!distracted);

        // Still on topic, just detailed
        let (distracted, pattern) = detector
            .detect_fixation("The learning process iteratively improves the model's predictions");

        println!(
            "Productive exploration test - distracted: {}, pattern: {:?}",
            distracted, pattern
        );
        println!("Relevance history: {:?}", detector.relevance_history);
        println!(
    ...
    fn test_no_original_concepts() {
        ...
        let (distracted, pattern) =
            detector.detect_fixation("Random thought about quantum computing");
        assert!(!distracted);
        assert!(matches!(pattern, DistractorPattern::None));
    }
}
