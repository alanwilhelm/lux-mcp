...
impl PlannerTool {
    ...
    pub async fn create_plan(&mut self, request: PlannerRequest) -> Result<PlannerResponse> {
        ...
        info!("Planner request - Step {}/{}, Model: {}, Temperature: {}, Session: {:?}", 
            request.step_number, request.total_steps, model, request.temperature, request.session_id);
        
        // Log processing status for long-running models
        if model.starts_with("o3") {
            info!("⏳ Using {} - this may take 30 seconds to 5 minutes. Processing...", model);
        }
        ...
        let generated_content = if request.step_number == 1 {
            ...
            let max_tokens = if model.starts_with("o3") { 32768 } else { 10000 };
            
            info!("🚀 Sending planning request to {} (max_tokens: {})", model, max_tokens);
            if model.starts_with("o3") {
            ...
            let response = client
                ...
                .map_err(|e| {
                    ...
                    error!("LLM call failed after {:?} for model '{}': {}", elapsed, model, e);
                    anyhow::anyhow!("Failed to generate planning step with model '{}' after {:?}: {}", model, elapsed, e)
                })?;
        ...
        // Monitor for circular reasoning
        {
            ...
            let signals = monitor_guard.analyze_thought(&generated_content, request.step_number as usize);
            
            if signals.circular_score > 0.7 {
                info!("Warning: Planning step {} may have circular reasoning", request.step_number);
            }
        }
    ...
    fn build_system_prompt(&self, request: &PlannerRequest) -> String {
        let base_prompt = "You are an expert planner helping to create a comprehensive, step-by-step plan. \
        ...
        if request.is_branch_point {
            ...
            format!("{}\n\nYou are REVISING a previous step. Improve upon it with new insights.", base_prompt)
        } else {
    ...
    fn build_planning_response(&self, request: &PlannerRequest, generated_content: String, model: &str) -> PlannerResponse {
        ...
        let next_steps = if request.next_step_required {
            ...
            Some(format!(
                ...
                request.step_number + 1, remaining_steps
            ))
        } else {
            None
        };
    ...
    fn add_deep_thinking_pause(&self, mut response: PlannerResponse, request: &PlannerRequest) -> PlannerResponse {
        ...
        let (required_thinking, next_steps) = match request.step_number {
            ...
            2 => {
                let thinking = vec![
                    "Evaluate the approach from step 1 - are there better alternatives?".to_string(),
                    ...
                    "Think about how different parts interconnect and affect each other".to_string(),
                ];
            ...
            3 => {
                let thinking = vec![
                    "Validate that the emerging plan addresses the original requirements".to_string(),
    ...
    fn generate_plan_summary(&self, request: &PlannerRequest) -> String {
        let mut summary = format!("COMPLETE PLAN: {} (Total {} steps completed)\n\n", request.step, request.total_steps);
        ...
        for (i, step) in self.step_history.iter().enumerate() {
            ...
            if step.is_revision {
                summary.push_str(&format!("  (Revised step {})\n", step.revises_step.unwrap_or(0)));
            }
